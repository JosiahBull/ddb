...
use dds::{threaded::controller as multi_threaded_controller, Dds};
...
fn test_large_file_duplicate() {
    ...
    let config = Dds {
        input: "test_large_file_duplicate-multi.bin".to_string(),
        output: "test_large_file_duplicate-multi.bin.copy".to_string(),
        threaded: true,
        generate: None,
    };

    // run the controller
    multi_threaded_controller(config).unwrap();
...
#[test]
fn test_multithreading_cli() {
    generate_test_file("test_multithreading_cli.bin");

    // hash the source file
    let start_hash: String = {
        let mut hasher = Sha256::new();
        let mut file = std::fs::File::open("test_multithreading_cli.bin").unwrap();
    ...
    Command::cargo_bin("dds")
        .unwrap()
        .arg("--input")
        .arg("test_multithreading_cli.bin")
        .arg("--output")
        .arg("test_multithreading_cli.bin.copy")
        .arg("--threaded")
        .write_stdin("y\n")
        .assert()
        .success();
    ...
    // validate the files are the same
    let mut file1 = std::fs::File::open("test_multithreading_cli.bin").unwrap();
    let mut file2 = std::fs::File::open("test_multithreading_cli.bin.copy").unwrap();
    ...
    let end_hash: String = {
        let mut hasher = Sha256::new();
        let mut file = std::fs::File::open("test_multithreading_cli.bin.copy").unwrap();
        std::io::copy(&mut file, &mut hasher).unwrap();
        format!("{:x}", hasher.finalize())
    };
    ...
    // remove the files
    std::fs::remove_file("test_multithreading_cli.bin").unwrap();
    std::fs::remove_file("test_multithreading_cli.bin.copy").unwrap();
}
